...
public class Manager {
	...
	public boolean updatePRStatusDatabase(int prIndex) {
		...
		try {
			updater.getStatement().execute("UPDATE `purchasereq`.`pr` SET `status` = '"+PurchaseReqSystem.pr.get(prIndex).getStatus()+"', `reason` = '"+PurchaseReqSystem.pr.get(prIndex).getReason()+"' WHERE (`PRID` = '"+PurchaseReqSystem.pr.get(prIndex).getPRID()+"')");
